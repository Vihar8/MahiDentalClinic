...
const submitPatientInquiry = async (req, res) => {
  try {
    ...
    // 1️⃣ Email TO CLINIC
    const clinicResponse = await resend.emails.send({
      from: `${process.env.CLINIC_NAME || "Clinic Website"} <onboarding@resend.dev>`,
      to: process.env.CLINIC_EMAIL,
      subject: `New Patient Inquiry – ${name}`,
      html: `
        <h2>New Patient Inquiry</h2>
    ...
    const patientResponse = await resend.emails.send({
      from: `${process.env.CLINIC_NAME || "Dental Clinic"} <onboarding@resend.dev>`,
      to: email,
      subject: "Thank you for contacting us",
      html: `
        <p>Dear ${name},</p>
